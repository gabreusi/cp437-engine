...
import {RAMP_MODES} from '../../render/ramp';
...
export const buildGroups = (world: World): MenuGroup[] => [
    {
        label: 'Camera',
        items: () => [
            slider({key: 'fovDegrees', label: 'FOV', min: 40, max: 110, step: 1, suffix: '°'}),
            slider({key: 'moveSpeed', label: 'Speed', min: 2, max: 60, step: 1}),
            slider({key: 'lookSensitivity', label: 'Sensibility', min: 0.0005, max: 0.006, step: 0.0001, digits: 4}),
        ],
    },
    {
        label: 'Grid',
        items: () => [
            slider({key: 'gridSize', label: 'Size', min: 1, max: 16, step: 0.5, digits: 1}),
            slider({key: 'viewDistance', label: 'View Distance', min: 60, max: 400, step: 10}),
            slider({key: 'gridGlow', label: 'Neon Glow', min: 0, max: 2, step: 0.02, digits: 2}),
            slider({key: 'groundReflectivity', label: 'Reflections', min: 0, max: 1, step: 0.02, digits: 2}),
    ...
    },
    {
        ...
        items: () => [
            ...
            slider({key: 'fogDensity', label: 'Density', min: 0, max: 2.5, step: 0.05, digits: 2}),
            slider({key: 'groundHaze', label: 'Ground Haze', min: 0, max: 1.5, step: 0.05, digits: 2}),
        ],
    },
    {
        label: 'Sun',
        items: () => [
            slider({key: 'sunElevation', label: 'Elevation', min: -20, max: 40, step: 0.5, digits: 1, suffix: '°'}),
            slider({key: 'sunAzimuth', label: 'Azimuth', min: -180, max: 180, step: 1, suffix: '°'}),
            slider({key: 'sunAngularSize', label: 'Size', min: 3, max: 30, step: 0.5, digits: 1, suffix: '°'}),
            slider({key: 'sunSlices', label: 'Slices', min: 0.2, max: 3, step: 0.1, digits: 1}),
            slider({key: 'sunLightIntensity', label: 'Light Intensity', min: 0, max: 3, step: 0.05, digits: 2}),
        ],
    },
    {
        label: 'Sky',
        items: () => [
            ...
            slider({key: 'skyReflectionIntensity', label: 'Sky Reflection', min: 0, max: 3, step: 0.05, digits: 2}),
        ],
    },
    {
        label: 'Graphics',
        items: () => [
            ...
            slider({key: 'ambientLevel', label: 'Ambient', min: 0, max: 1, step: 0.02, digits: 2}),
            slider({key: 'maxShadowLights', label: 'Max Shadow Lights', min: 0, max: 8, step: 1}),
            {kind: 'heading', label: 'Character Set'},
            {
                kind: 'choice',
                label: 'Ramp',
                options: RAMP_MODES.map((mode) => ({value: mode, label: RAMP_LABELS[mode] ?? mode})),
                get: () => settings.glyphRamp,
                set: (value) => {
                    settings.glyphRamp = value as Settings['glyphRamp'];
                },
            },
            slider({key: 'rampExposure', label: 'Exposure', min: 0.1, max: 3, step: 0.05, digits: 2}),
        ],
    },
    {
        label: 'Objects',
    ...
    },
    {
        label: 'VFX',
        items: () => [
            slider({key: 'bloomIntensity', label: 'Bloom', min: 0, max: 2, step: 0.05, digits: 2}),
            slider({key: 'bloomRadius', label: 'Bloom Radius', min: 0.5, max: 6, step: 0.1, digits: 1}),
            slider({key: 'scanlineStrength', label: 'Scanlines', min: 0, max: 0.6, step: 0.02, digits: 2}),
            slider({key: 'vignetteStrength', label: 'Vignette', min: 0, max: 1, step: 0.05, digits: 2}),
...
const buildObjectItems = (world: World): MenuItem[] => {
    ...
    if (selected !== null) {
        items.push({kind: 'heading', label: selected.name.toUpperCase()});

        for (const field of ENTITY_KINDS[selected.kind].fields) {
            items.push({
                kind: 'slider',
                label: field.label,
                min: field.min,
                ...
                digits: field.digits,
                suffix: field.suffix,
                get: () => field.get(selected),
                set: (value) => field.set(selected, value),
            });
        }
        ...
        items.push({
            kind: 'toggle',
            label: 'Cast shadows',
            get: () => selected.castsShadow,
            set: (value) => {
                selected.castsShadow = value;
            },
        });
        items.push({
            kind: 'toggle',
