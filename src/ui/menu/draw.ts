...
export const drawMenu = (
    ...
    drawFill(framebuffer, col, row, width, height, GLYPH.BLOCK_FULL, MENU_COLORS.BACKDROP, BACKDROP_ALPHA);
    drawBox(framebuffer, col, row, width, height, MENU_COLORS.BORDER);

    drawText(framebuffer, col + 2, row, ' CP437 ENGINE', MENU_COLORS.TITLE, 1, 0.4);
    ...
    plot(framebuffer, dividerCol, row + height - 1, GLYPH.BOX_HU, MENU_COLORS.BORDER);
...
const drawGroups = (framebuffer: Framebuffer, layout: MenuLayout, state: DrawState): void => {
    for (let index = 0; index < state.groups.length; index += 1) {
        ...
        const color = active
            ? (state.onGroups ? MENU_COLORS.FOCUS : MENU_COLORS.VALUE)
            : MENU_COLORS.LABEL;

        if (active) plot(framebuffer, layout.groupCol - 1, y, GLYPH.ARROW_RIGHT, color, 0.3);
        drawText(framebuffer, layout.groupCol + 1, y, state.groups[index]!.label, color, 1, active ? 0.3 : 0);
    }
};

const drawItems = (framebuffer: Framebuffer, layout: MenuLayout, state: DrawState): void => {
    ...
    if (state.scroll > 0) {
        plot(framebuffer, layout.itemCol + layout.itemWidth, layout.itemFirstRow,
            GLYPH.ARROW_UP, MENU_COLORS.DIM);
    }
    if (end < state.items.length) {
        plot(framebuffer, layout.itemCol + layout.itemWidth,
            layout.itemFirstRow + layout.visibleRows - 1, GLYPH.ARROW_DOWN, MENU_COLORS.DIM);
    }
};
...
const drawItem = (
    ...
    if (focused) plot(framebuffer, itemCol - 1, y, GLYPH.ARROW_RIGHT, MENU_COLORS.FOCUS, emissive);

    switch (item.kind) {
        case 'slider': {
            drawText(framebuffer, itemCol, y, item.label.slice(0, LABEL_WIDTH), labelColor, 1, emissive);
        ...
        case 'toggle': {
            const on = item.get();
            drawText(framebuffer, itemCol, y, on ? '[x]' : '[ ]',
                on ? MENU_COLORS.VALUE : MENU_COLORS.DIM, 1, emissive);
            drawText(framebuffer, itemCol + 4, y, item.label, labelColor, 1, emissive);
            return;
        }
        ...
        case 'choice': {
            drawText(framebuffer, itemCol, y, item.label.slice(0, LABEL_WIDTH), labelColor, 1, emissive);
            plot(framebuffer, trackCol - 1, y, GLYPH.ARROW_LEFT, MENU_COLORS.DIM);
            const current = item.options.find((option) => option.value === item.get());
            drawText(framebuffer, trackCol + 1, y, current?.label ?? '?', MENU_COLORS.VALUE, 1, emissive);
            plot(framebuffer, trackCol + trackWidth, y, GLYPH.ARROW_RIGHT, MENU_COLORS.DIM);
            return;
        }
        ...
        case 'action': {
            const color = item.danger
                ? (focused ? MENU_COLORS.DANGER : MENU_COLORS.DANGER)
                : labelColor;
            drawText(framebuffer, itemCol, y, `» ${item.label}`, color, item.danger && !focused ? 0.7 : 1, emissive);
            return;
        }

        case 'entity': {
            drawText(framebuffer, itemCol, y, item.visible ? '[x]' : '[ ]',
                item.visible ? MENU_COLORS.VALUE : MENU_COLORS.DIM, 1, emissive);

            const color = item.selected ? MENU_COLORS.VALUE : labelColor;
            drawText(framebuffer, itemCol + 4, y, item.label, color, 1,
                item.selected ? Math.max(emissive, 0.3) : emissive);

            if (item.selected) {
                plot(framebuffer, itemCol + layout.itemWidth - 1, y, GLYPH.DIAMOND, MENU_COLORS.FOCUS, 0.3);
            }
            return;
        }
    }
