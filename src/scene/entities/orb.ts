import { type Rgb, copyRgb, luminance, rgb, scaleRgb } from '../../math/color';
import { copy } from '../../math/vec3';
import { LIGHT, OCCLUDER } from '../../light/types';
import type { LightWorld } from '../../light/world';
import { glyphForLuminance } from '../../render/ramp';
...
export const orbKind: EntityKindDef = {
    ...
    render: (entity: EntityState, context: RenderContext, pen: SurfacePen): void => {
        ...
        const base = luminance(entity.color) * Math.max(0.4, entity.intensity * 0.28);
        ...
        rasterizer.disc(projected, radiusRows, rasterizer.lastRow, (col, row, nx, ny) => {
            ...
            rasterizer.plotCell(
                col, row,
                glyphForLuminance(brightness, 0, '#'.charCodeAt(0), rampMode, rampExposure),
                tint,
                depth,
                1,
                peak - 1,
    ...
    fields: [
        ...COMMON_FIELDS,
        {
            label: 'Radius', min: 0.2, max: 8, step: 0.1, digits: 1,
            get: (e) => e.size.x,
            set: (e, v) => { e.size.x = v; e.size.y = v; e.size.z = v; },
        },
        {
            label: 'Glow intensity', min: 0, max: 40, step: 0.5, digits: 1,
            get: (e) => e.intensity,
            set: (e, v) => { e.intensity = v; },
        },
        {
            label: 'Range', min: 2, max: 160, step: 2,
            get: (e) => e.range,
            set: (e, v) => { e.range = v; },
        },
        ...colorFields(),
        {
            label: 'Orbit Radius', min: 0, max: 60, step: 0.5, digits: 1,
            get: (e) => e.orbitRadius,
            set: (e, v) => { e.orbitRadius = v; },
        },
        {
            label: 'speed', min: -2, max: 2, step: 0.05, digits: 2,
            get: (e) => e.orbitSpeed,
            set: (e, v) => { e.orbitSpeed = v; },
        },
    ],
};
