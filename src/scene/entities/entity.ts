...
export const createEntity = (kind: EntityKind, overrides: Partial<EntityState> = {}): EntityState => ({
    id: nextId++,
    kind,
    name: kind,
    visible: true,
...
export const COMMON_FIELDS: readonly EntityField[] = [
    {
        label: 'X', min: -120, max: 120, step: 0.5, digits: 1,
        get: (e) => e.position.x,
        set: (e, v) => { e.position.x = v; },
    },
    {
        label: 'Y', min: -20, max: 60, step: 0.5, digits: 1,
        get: (e) => e.position.y,
        set: (e, v) => { e.position.y = v; },
    },
    {
        label: 'Z', min: -200, max: 120, step: 0.5, digits: 1,
        get: (e) => e.position.z,
        set: (e, v) => { e.position.z = v; },
    },
];
...
export const ROTATION_FIELDS: readonly EntityField[] = [
    {
        label: 'Rotation', min: -180, max: 180, step: 1, suffix: '°',
        get: (e) => e.yaw * DEGREES,
        set: (e, v) => { e.yaw = v / DEGREES; },
    },
    {
        label: 'Yaw', min: -90, max: 90, step: 1, suffix: '°',
        get: (e) => e.pitch * DEGREES,
        set: (e, v) => { e.pitch = v / DEGREES; },
    },
];

export const colorFields = (): readonly EntityField[] => [
    {
        label: 'Red', min: 0, max: 1, step: 0.02, digits: 2,
        get: (e) => e.color.r,
        set: (e, v) => { e.color.r = v; },
    },
    {
        label: 'Green', min: 0, max: 1, step: 0.02, digits: 2,
        get: (e) => e.color.g,
        set: (e, v) => { e.color.g = v; },
    },
    {
        label: 'Blue', min: 0, max: 1, step: 0.02, digits: 2,
        get: (e) => e.color.b,
        set: (e, v) => { e.color.b = v; },
    },
];

export const sizeFields = (labels: readonly [string, string, string]): readonly EntityField[] => [
    {
        label: labels[0], min: 0.2, max: 30, step: 0.2, digits: 1,
        get: (e) => e.size.x,
        set: (e, v) => { e.size.x = v; },
    },
    {
        label: labels[1], min: 0.2, max: 30, step: 0.2, digits: 1,
        get: (e) => e.size.y,
        set: (e, v) => { e.size.y = v; },
    },
    {
        label: labels[2], min: 0.2, max: 30, step: 0.2, digits: 1,
        get: (e) => e.size.z,
        set: (e, v) => { e.size.z = v; },
    },
];
