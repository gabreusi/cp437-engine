import { type Rgb, luminance, rgb, setRgb } from '../math/color';
import { skyRadiance } from './sky';
import { SHADOW_BIAS, occluded, traceNearest } from './trace';
...
export const shadeSurface = (
    ...
    for (let index = 0; index < world.lightCount; index += 1) {
        ...
        if (light.kind === LIGHT.DIRECTIONAL) {
            ...
            attenuation = (window * window * HALF_POWER_SQ) / (distanceSq + HALF_POWER_SQ);
        ...
        if (
            ...
            shadowRays += 1;
            if (occluded(world, originX, originY, originZ, lx, ly, lz, distance, ownerId)) {
                continue;
            }
        }
    ...
    if (options.reflections && material.reflectivity > 0) {
        ...
        const hit = material.mirror
            ? traceNearest(world, originX, originY, originZ, rx, ry, rz, MIRROR_RANGE, ownerId)
            : null;
