...
export interface SurfaceSample {
   ...
   * forma (`glyph-shape.ts`) saber não só para onde a linha aponta, mas por
   * onde ela passa.
   */
  offsetCol: number;
  offsetRow: number;
  /**
   * Direção do segmento em espaço de tela, unitária, em unidades de
   * coluna/fileira — sem correção de `CELL_ASPECT`. As amostras do atlas
   * também vivem nessa caixa 1:2 da célula, então já concordam sem reescalar
   * (ver `glyph-shape.ts`).
   */
  dirCol: number;
  dirRow: number;
...
/** Abaixo disto o segmento é um ponto e a direção não significa nada. */
...
export class Rasterizer {
  ...
  private drawScreenLine(
    ...
    const wa0 = this.worldA.x * invW0,
      wa1 = this.worldB.x * invW1;
    const wb0 = this.worldA.y * invW0,
      wb1 = this.worldB.y * invW1;
    const wc0 = this.worldA.z * invW0,
      wc1 = this.worldB.z * invW1;
    ...
    const steps = Math.max(
      1,
      Math.ceil(Math.max(Math.abs(spanCol), Math.abs(spanRow))),
    );

    // Direção unitária do segmento, uma vez por chamada — igual à antiga
    // `slope`, só que contínua em vez de quatro baldes. Sem correção de
    // `CELL_ASPECT`: ver o comentário de `dirCol`/`dirRow` em `SurfaceSample`.
    const dirLength = Math.hypot(spanCol, spanRow);
    const dirCol = dirLength < DIRECTION_EPSILON ? 1 : spanCol / dirLength;
    const dirRow = dirLength < DIRECTION_EPSILON ? 0 : spanRow / dirLength;

    const sample = this.sample;
    sample.dirCol = dirCol;
    sample.dirRow = dirRow;
  ...
  disc(
    ...
    const maxCol = Math.min(
      Math.ceil(radiusCols),
      this.colCount - 1 - centerCol,
    );
  ...
  horizonRow(): number {
    return (
      (0.5 + Math.tan(this.camera.pitch) * this.focalY * 0.5) * this.rowCount
    );
  }

  /** Nada foi escrito nesta célula ainda. Ver `Framebuffer.isEmpty`. */
  cellIsEmpty(col: number, row: number): boolean {
    return this.framebuffer.isEmpty(col, row);
  }
